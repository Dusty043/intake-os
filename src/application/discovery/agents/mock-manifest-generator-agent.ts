...
function generateReadme(proposal: ProjectProposal, session: DiscoverySession): GitHubReadme {
  const lines: string[] = [];
  ...
  lines.push("## Links");
  lines.push("");
  lines.push("- Project Intake OS record: _TBD after intake is created_");
  lines.push("");
  lines.push("---");
...
export class MockManifestGeneratorAgent implements IManifestGeneratorAgent {
  async generateManifest(
    ...
    const github: GitHubManifest = {
      ...
      labels: issueLabels,
      readme: needsRepo ? generateReadme(proposal, session) : null,
