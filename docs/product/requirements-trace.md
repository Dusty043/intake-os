...
| P0-009 | Critic reviews complete evaluation evidence and low scores receive one bounded feedback-guided repair pass | `ai-orchestration.md`, `ai-cost-governance.md` | `OpenAICriticQAAgent`, `OpenAIWorkBreakdownAgent`, `PhaseZeroPacketService` | TASK-0083 | `tests/phase-zero-packet.test.mjs`, PhaseZeroPacketPanel test | tested | Critic and full-depth work-breakdown responses reserve schema-specific headroom. Repair failure preserves the original downloadable packet; users may explicitly improve `ready_with_warnings`, while `ready` remains immutable. |
