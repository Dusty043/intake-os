...
| B-018 | Unconfirmed discovery assumptions are surfaced to Intake for active confirmation, not silently accepted | `ai-orchestration.md` | `proposal-to-intake-adapter.ts`, `OpenAIClarificationQuestionsAgent` | TASK-0076 | `tests/proposal-to-intake-adapter-assumptions.test.mjs` (3 tests) | tested | `proposal.assumptions` (guessed by discovery at the `propose_with_assumptions` confidence tier or via the "wing it" escape valve, without ever asking the user) were previously dropped entirely when building the intake record — only `proposal.unknowns` reached `discovery.notes`. Now included as a distinctly-labeled section; the clarification agent's prompt instructs it to actively confirm consequential assumptions rather than accept them, per user direction to "take the safest road" since live AI is already in the loop. Mock-provider path unaffected (Q-DISC-1). |
| B-019 | Every discovery assumption carries a required rationale, not a bare claim | `ai-orchestration.md` | `DiscoveryAssumption` type (`domain/discovery.ts`), `openai-problem-framing-agent.ts`, `openai-proposal-composer-agent.ts` | TASK-0077 | `tests/discovery-phase-1.test.mjs` (rationale non-empty), `tests/proposal-to-intake-adapter-assumptions.test.mjs` (rationale reaches notes) | tested | `assumptions: string[]` → `DiscoveryAssumption[] = {assumption, rationale}[]` across `ProblemFrame` and `ProjectProposal`; both OpenAI agent JSON schemas mark `rationale` `required` with `additionalProperties: false` so the model cannot omit it. Every producer/consumer updated (mock agents, manifest generator, chat reply text, intake adapter, frontend type mirror). User's framing: "the ai is solving the gap between what is known and not, and providing a safe way to achieve it" — rationale is where that safety reasoning must be stated. |
| B-020 | On the orchestrator path, the evaluation is the single reviewable artifact — no derived draft twin | `ai-orchestration.md` | `evaluation-reviewed-package.ts`, `intake-workflow-service.ts` (eval-based accept/reject/revise/regenerate), `intake.controller.ts` (`:id/evaluation/*`) | TASK-0078 | `tests/generate-evaluation-service.test.mjs` (rewritten to assert on the evaluation) | tested | A-scoped: `generateEvaluation` no longer down-converts to an `IntakeAnalysisDraft`; the reviewed package is built directly from evaluation sections via `evaluationToReviewedPackage`, and governance operates on the evaluation's own `status` lifecycle. The single-call provider engine + its draft path are retained as a fallback (draft still shown when an evaluation is absent). UI collapses "AI Draft" + "Evaluation" into one review surface. `evaluation-draft-mapper.ts` now unused (barrel export only) — future cleanup. |
...
| G-002 | Lower-cost models used for summarization, classification, clarification, extraction | `ai-cost-governance.md` | Model router | TASK-0030, TASK-0056 | `tests/analysis-provider-config.test.mjs` | implemented | TASK-0056 (2026-07-11) wired real per-agent tiering — previously every Discovery and Evaluation agent shared one model regardless of this table. `resolveTasksModel()` now routes Intake Analyst, Clarification, Project Classifier, Low-Code Path, Distribution Planner (Evaluation) and Intent Extraction, Problem Framing, Clarification (Discovery) to the lower-cost tier (`gpt-5.6-terra` by default). No per-agent-routing test exists yet — only config resolution is tested; routing correctness was verified by reading the updated call sites. |
| G-003 | Higher-capability models used for architecture, planning, trade-offs, issue generation | `ai-cost-governance.md` | Model router | TASK-0030, TASK-0056 | `tests/analysis-provider-config.test.mjs` | implemented | TASK-0056: `resolveModel()` routes Solutions Architect, Custom Build, Risk/Security, Cost/Effort, Work Breakdown, Final Synthesis, Critic/QA (Evaluation) and Solution Generation, Proposal Composition (Discovery) to the higher-capability tier (`gpt-5.6-sol` by default), plus the single-call intake analysis pipeline (unchanged from TASK-0036's decision). |
...
| G-007 | Cost optimization prefers task-appropriate models and section regeneration | `ai-cost-governance.md` | AI orchestration | TASK-0030, TASK-0056 | routing/regeneration tests | in_progress | Task-appropriate model routing now implemented (see G-002/G-003); section regeneration cost-awareness still unimplemented. |
...
| P0-009 | Critic reviews complete evaluation evidence and low scores receive one bounded feedback-guided repair pass | `ai-orchestration.md`, `ai-cost-governance.md` | `OpenAICriticQAAgent`, `OpenAIWorkBreakdownAgent`, `OpenAIFinalSynthesisAgent`, `PhaseZeroPacketService` | TASK-0083 | `tests/phase-zero-packet.test.mjs`, PhaseZeroPacketPanel test | tested | Critic, full-depth work-breakdown, and final-synthesis responses reserve schema-specific headroom. Repair failure preserves the original downloadable packet; users may explicitly improve `ready_with_warnings`, while `ready` remains immutable. |
